...
function setupLocal() {
  localStrategy = new LocalStrategy(function (
    username,
    plaintextPassword,
    done
  ) {
    ...
    if (found === undefined) return done(null, null);
    else {
...
async function setupAuth() {
  ...
  app.get("/auth/check_login_status", (req, res) => {
    ...
    if (Boolean(req.session.passport) && Boolean(req.session.passport.user)) {
      loggedIn = true;
    }
...
async function runHttpsServer() {
  ...
  app.use(
    ...
    express.static("dist/public/.well-known/acme-challenge")
  );
    ...
    queue
      .push(async () => {
        ...
        if (
          ...
          const { id, displayName, picture, email, _userinfo } =

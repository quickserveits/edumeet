...
if (!configLoaded) {
  logger.warn(
    `No config file found in ${path.normalize(
      `${__dirname}/../../config/`
    )}, using defaults.`
  );
  configSchema.load({});
...
const configModuleFilepath = path.normalize(
...
if (fs.existsSync(configModuleFilepath)) {
  try {
    ...
    logger.error(
      `Error loading ${configModuleFilepath} module: ${err.message}`
    );
  }
}
...
//
export { configSchema, config, configError, configDocs };

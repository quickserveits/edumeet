...
class Room extends EventEmitter {
  ...
  _handleLobby() {
    ...
    this._lobby.on("peerClosed", (closedPeer) => {
      ...
      const { id, displayName } = closedPeer;

      console.log('closedPeer', closedPeer);

      for (const peer of this._getAllowedPeers(PROMOTE_PEER)) {
        this._notification(peer.socket, "lobby:peerClosed", { peerId: id, displayName: displayName});
      }
    });
  ...
  _handlePeerClose(peer) {
    ...
    // this._notification(peer.socket, "peerClosed", { peerId: peer.id, displayName: peer.displayName }, true);
  ...
  async _handleSocketRequest(peer, request, cb) {
    ...
    switch (request.method) {
      ...
      case "Host:clearChat": {
        ...
        // Spread to others
        this._notification(peer.socket, "Host:clearChat", null, true);
      ...
      case "Host:mute": {
        ...
        this._notification(mutePeer.socket, "Host:mute");
      ...
      case "Host:muteAll": {
        ...
        // Spread to others
        this._notification(peer.socket, "Host:mute", null, true);
      ...
      case "Host:stopVideo": {
        ...
        this._notification(stopVideoPeer.socket, "Host:stopVideo");
      ...
      case "Host:stopAllVideo": {
        if (!this._hasPermission(peer, MODERATE_ROOM))
          throw new Error("peer not authorized");

        // Spread to others
        this._notification(peer.socket, "Host:stopVideo", null, true);
      ...
      case "Host:stopAllScreenSharing": {
        if (!this._hasPermission(peer, MODERATE_ROOM))
          throw new Error("peer not authorized");

        // Spread to others
        this._notification(peer.socket, "Host:stopScreenSharing", null, true);
      ...
      case "Host:stopScreenSharing": {
        ...
        this._notification(stopVideoPeer.socket, "Host:stopScreenSharing");
      ...
      case "Host:closeMeeting": {
        if (!this._hasPermission(peer, MODERATE_ROOM))
          throw new Error("peer not authorized");

        this._notification(peer.socket, "Host:kick", null, true);
      ...
      case "Host:kickPeer": {
        ...
        this._notification(kickPeer.socket, "Host:kick");
      ...
      case "Host:lowerHand": {
        ...
        this._notification(lowerPeer.socket, "Host:lowerHand");
